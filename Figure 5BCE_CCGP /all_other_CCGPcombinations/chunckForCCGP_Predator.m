...
%% Get neruon numbers and indeces for each patient
...
Rpred_global_pt = cell(numel(DMcat),1);
...
for d = 1:numel(DMcat)
    ...
    for iN = 1:N
        ...
        [R, ~] = rate_map_with_occ(predX, predY, spikes, Edges(d).xedges, Edges(d).yedges, dt);
        Rpred_global(iN,:) = reshape(R,1,[]);
    end

    Rself_global_pt{d} = Rself_global;
    Rprey_global_pt{d} = Rprey_global;
    Runprey_global_pt{d} = Runprey_global;
    Rpred_global_pt{d} = Rpred_global;
end
...
for k = 1:maxChunks
    ...
    for d = 1:numel(DMcat)
        ...
        [~, OccPr] = rate_map_with_occ(predX(idx), predY(idx), spikes_dummy, xedges, yedges, dt);
        ...
        for iN = 1:N
            ...
            [R, ~] = rate_map_with_occ(predX(idx), predY(idx), spikes, xedges, yedges, dt);
            r = reshape(R,1,[]);
            Rpred_c(iN,:) = wPr .* r + (1-wPr) .* Rpred_global(iN,:);
        end
...
for k = 1:K
    ...
    good_hi_Pr = bins_hi(OccPr(bins_hi) >= min_occ);
    good_lo_Pr = bins_lo(OccPr(bins_lo) >= min_occ);
    ...
    if numel(good_hi_Pr) < min_bins_class || numel(good_lo_Pr) < min_bins_class, continue; end

    % --- decide how many bins per class per chunk (balanced + capped) ---
    nS = min([numel(good_hi_S), numel(good_lo_S), max_bins_per_class_per_chunk]);
    nP = min([numel(good_hi_P), numel(good_lo_P), max_bins_per_class_per_chunk]);
    nU = min([numel(good_hi_U), numel(good_lo_U), max_bins_per_class_per_chunk]);
    nPr = min([numel(good_hi_Pr), numel(good_lo_Pr), max_bins_per_class_per_chunk]);
    ...
    % Predator
    occ_hi_Pr = OccPr(good_hi_Pr);
    occ_lo_Pr = OccPr(good_lo_Pr);
    [~, ord_hi_Pr] = sort(occ_hi_Pr, 'descend');
    [~, ord_lo_Pr] = sort(occ_lo_Pr, 'descend');
    good_hi_Pr = good_hi_Pr(ord_hi_Pr(1:nPr));
    good_lo_Pr = good_lo_Pr(ord_lo_Pr(1:nPr));
    ...
    PredItems_k = zscore_items_by_pt_blocks(PredItems_k, ptStart, ptEnd);
    ...
    PredItems_k(isnan(PredItems_k)) = 0;
    ...
    PredItems = [PredItems; PredItems_k];
    yPred = [yPred; ypr];
    chunk_id_pred = [chunk_id_pred; k*ones(size(PredItems_k,1),1)];

end

fprintf('Built items: Self=%d x %d | Prey=%d x %d\n | Unprey=%d x %d\n | Predator=%d x %d\n',  ...
    size(SelfItems,1), size(SelfItems,2), size(PreyItems,1), size(PreyItems,2), size(UnpreyItems,2), size(PredItems,2));
...
fprintf('Class balance: Self high=%d low=%d | Prey high=%d low=%d\n | Unprey high=%d low=%d\n | Predator high=%d low=%d\n', ...
    sum(ySelf==1), sum(ySelf==0), sum(yPrey==1), sum(yPrey==0), sum(yUnprey==1), sum(yUnprey==0), sum(yPred==1), sum(yPred==0));


%% Save data for CCGP decoding 
save('chunckForCCGP_40s_6BESTbinsPerClassPredators.mat', 'SelfItems', 'PreyItems', 'UnpreyItems', 'PredItems', 'ySelf', 'yPrey', 'yUnprey', 'yPred');
