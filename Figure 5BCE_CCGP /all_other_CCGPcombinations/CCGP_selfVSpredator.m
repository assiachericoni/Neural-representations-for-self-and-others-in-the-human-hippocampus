...
load('chunckForCCGP_40s_6BESTbinsPerClassPredators.mat')
...
high_pred = find(yPred == 1);
low_pred  = find(yPred == 0);

acc_real_pred   = nan(n_reps,1);
...
for rep = 1:n_reps
    ...
    if numel(high_pred) < nHte || numel(low_pred) < nLte
        % not enough prey samples for this rep
        continue;
    end
    
    th_pred = high_pred(randperm(numel(high_pred), nHte));
    tl_pred = low_pred(randperm(numel(low_pred),  nLte));
    
    idx_test_pred = [th_pred; tl_pred];
    X_test_pred   = PredItems(idx_test_pred, :);
    y_test_pred   = [ones(numel(th_pred),1); zeros(numel(tl_pred),1)];
    
    % apply SELF training z-score to prey
    X_test_pred_z = (X_test_pred - mu) ./ sigma;
    
    yhat_pred = predict(mdl, X_test_pred_z);
    acc_real_pred(rep) = mean(yhat_pred == y_test_pred);
    
end

CCGP_real_pred  = mean(acc_real_pred,   'omitnan')
...
acc_shuf_pred = nan(n_shuffles,1);
...
for sh = 1:n_shuffles
    ...
    yPr = yPred(randperm(numel(yPred)));

    high_self_sh = find(yS==1);  low_self_sh  = find(yS==0);
    high_pred_sh = find(yPr==1);  low_pred_sh  = find(yPr==0);

    acc_rep_pred = nan(n_reps,1);
    ...
    parfor rep = 1:n_reps
        ...
        % ============================
        % 2) TEST ON SHUFFLED Predator
        % ============================
        if numel(high_pred_sh) < nHte || numel(low_pred_sh) < nLte
            % not enough gaze samples in this shuffle, skip this rep
            continue;
        end

        th_g = high_pred_sh(randperm(numel(high_pred_sh), nHte));
        tl_g = low_pred_sh(randperm(numel(low_pred_sh),  nLte));

        idx_test_pred = [th_g; tl_g];
        X_test_pred   = PredItems(idx_test_pred, :);
        y_test_pred   = [ones(numel(th_g),1); zeros(numel(tl_g),1)];

        X_test_pred_z = (X_test_pred - mu) ./ sigma;

        yhat_pred = predict(mdl, X_test_pred_z);
        acc_rep_pred(rep) = mean(yhat_pred == y_test_pred);
    end

    acc_shuf_pred(sh) = mean(acc_rep_pred,'omitnan');
end

% remove NaN shuffles if any reps were skipped
acc_shuf_pred = acc_shuf_pred(~isnan(acc_shuf_pred));

% p-value vs real CCGP
pval_pred = mean(acc_shuf_pred >= CCGP_real_pred);

fprintf('SELF->GAZE CCGP: real=%.3f | shuffle=%.3f±%.3f | p=%.4f\n', ...
    CCGP_real_pred, mean(acc_shuf_pred), std(acc_shuf_pred), pval_pred);

% visualize
figure; hold on;
histogram(acc_shuf_pred,'Normalization','probability');
yl = ylim;
plot([CCGP_real_pred CCGP_real_pred], yl, 'k-', 'LineWidth', 2);
plot([0.5 0.5], yl, 'k--', 'LineWidth', 1);
title(sprintf('Self->Predator: real=%.3f, p=%.4f', CCGP_real_pred, pval_pred));
...
real_mean = mean(acc_real_pred,   'omitnan');
null_mean = mean(acc_shuf_pred,   'omitnan');
null_std  = std( acc_shuf_pred,   'omitnan');
...
jitter = 0.12;
xj = xpos + (rand(size(acc_real_pred))-0.5)*2*jitter;
scatter(xj, acc_real_pred, 25, ...
...
% cosmetics
xlim([0.5 1.5]);
set(gca, 'XTick', xpos, 'XTickLabel', {'Self \rightarrow Predator'});
ylabel('Cross-agent generalization accuracy');
title('Train Self, test Predator');
...
% --- summarize real and shuffle distributions ---

real_means = [mean(acc_real_self,   'omitnan'), ...
    mean(acc_real_prey, 'omitnan'),...
    mean(acc_real_unprey, 'omitnan')];

null_means = [mean(acc_shuf_self,   'omitnan'), ...
    mean(acc_shuf_prey, 'omitnan'),...
    mean(acc_shuf_unprey, 'omitnan')];

null_stds  = [std(acc_shuf_self,   'omitnan'), ...
    std(acc_shuf_prey , 'omitnan'), ...
    std(acc_shuf_unprey, 'omitnan')];

xpos = [1 2 3];   % 1 = chosen prey, 2 = unchosen prey
...
% 1) plot shuffle null (mean ± 1 SD) as thick black vertical bars
for i = 1:numel(xpos)
    x = xpos(i);
    m = null_means(i);
    s = null_stds(i);

    % vertical bar from (m - s) to (m + s)
    plot([x x], [m-s, m+s], 'k-', 'LineWidth', 4);

    % optional thick horizontal caps at top & bottom
    cap_width = 0.15;
    plot([x-cap_width, x+cap_width], [m-s, m-s], 'k-', 'LineWidth', 4);
    plot([x-cap_width, x+cap_width], [m+s, m+s], 'k-', 'LineWidth', 4);
end

% 2) scatter real CAGP accuracies (each repetition) as gray dots
jitter = 0.12;

% Gazer -> self
xj = xpos(1) + (rand(size(acc_real_self))-0.5)*2*jitter;
scatter(xj, acc_real_self, 25, 'MarkerFaceColor', col_dots, ...
    'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.8, 'MarkerEdgeAlpha', 0.6);

% Gaze -> chosen prey
xj = xpos(2) + (rand(size(acc_real_prey))-0.5)*2*jitter;
scatter(xj, acc_real_prey, 25, 'MarkerFaceColor', col_dots, ...
    'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.8, 'MarkerEdgeAlpha', 0.6);

% Gaze -> unchosen prey
xj = xpos(3) + (rand(size(acc_real_unprey))-0.5)*2*jitter;
scatter(xj, acc_real_unprey, 25, 'MarkerFaceColor', col_dots, ...
    'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.8, 'MarkerEdgeAlpha', 0.6);
...
yl = ylim;
plot([0.5 2.5 3.5], [0.5 0.5 0.5], 'k--', 'LineWidth', 1.5);
ylim(yl);   % restore if plot resized ylim; or set manually, e.g. [0.2 0.9]
