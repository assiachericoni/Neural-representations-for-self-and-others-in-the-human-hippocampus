%% Load updated DMcat with distance-matched indices
close all; clear

decoding = 'rightleft'; % 'topbottom' or 'diagonal' or 'rightleft'

%load("/Users/assiachericoni/Documents/MATLAB/data/PacMan/cumulativeAll/self-chosen-unchosenCCGPcontrol/DMcatAll_with_distance_indeces_thr.mat")

load("/Users/assiachericoni/Documents/MATLAB/data/PacMan/cumulativeAll/self-chosen-unchosenCCGPcontrol/DMcatAll_with_distance_indeces.mat")
%% Get neuron numbers and indeces for each patient
...
%% Data chunking and psudopopulation
...
OccPreyList   = {};   % each cell k: [36 x 1]
...
% ptNneurons = zeros(numel(DMcat),1);
...
for d = 1:numel(DMcat)
    ...
    idx_prey_global    = DM.self_chosen_distmatched_idx(:);
    idx_unprey_global  = DM.self_unchosen_distmatched_idx(:);

    for iN = 1:N
        spikes = spk(:,iN);

        % self: all samples
        [R, ~] = rate_map_with_occ(selfX, selfY, spikes, ...
            Edges(d).xedges, Edges(d).yedges, dt);
        Rself_global(iN,:) = reshape(R,1,[]);

        % chosen prey: distance-matched samples only
        spikes_prey = spk(idx_prey_global,iN);
        [R, ~] = rate_map_with_occ(preyX(idx_prey_global), preyY(idx_prey_global), ...
            spikes_prey, Edges(d).xedges, Edges(d).yedges, dt);
        Rprey_global(iN,:) = reshape(R,1,[]);

        % unchosen prey: distance-matched samples only
        spikes_unprey = spk(idx_unprey_global,iN);
        [R, ~] = rate_map_with_occ(unpreyX(idx_unprey_global), unpreyY(idx_unprey_global), ...
            spikes_unprey, Edges(d).xedges, Edges(d).yedges, dt);
...
for k = 1:maxChunks
    ...
    for d = 1:numel(DMcat)
        ...
        idx_all = (k-1)*chunk_len + (1:chunk_len);

        mask_prey   = DM.self_chosen_distmatched_mask(:);
        mask_unprey = DM.self_unchosen_distmatched_mask(:);

        idx_prey   = idx_all(mask_prey(idx_all));
        idx_unprey = idx_all(mask_unprey(idx_all));

        if numel(idx_prey) < 100 || numel(idx_unprey) < 100
            continue
        end

        % get position occupancy only - no spikes here - using the k chunck of samples
        % self: all samples
        spikes_dummy_self = zeros(numel(idx_all),1);
        [~, OccS] = rate_map_with_occ(selfX(idx_all), selfY(idx_all), ...
            spikes_dummy_self, xedges, yedges, dt);

        % chosen prey: matched samples only
        spikes_dummy_prey = zeros(numel(idx_prey),1);
        [~, OccP] = rate_map_with_occ(preyX(idx_prey), preyY(idx_prey), ...
            spikes_dummy_prey, xedges, yedges, dt);

        % unchosen prey: matched samples only
        spikes_dummy_unprey = zeros(numel(idx_unprey),1);
        [~, OccU] = rate_map_with_occ(unpreyX(idx_unprey), unpreyY(idx_unprey), ...
            spikes_dummy_unprey, xedges, yedges, dt);
        ...
        for iN = 1:N
            % self: all chunk samples
            spikes_self = spk(idx_all,iN);

            [R, ~] = rate_map_with_occ(selfX(idx_all), selfY(idx_all), ...
                spikes_self, xedges, yedges, dt);
            r = reshape(R,1,[]);
            Rself_c(iN,:) = wS .* r + (1-wS) .* Rself_global(iN,:);

            % chosen prey: matched samples only
            spikes_prey = spk(idx_prey,iN);

            [R, ~] = rate_map_with_occ(preyX(idx_prey), preyY(idx_prey), ...
                spikes_prey, xedges, yedges, dt);
            r = reshape(R,1,[]);
            Rprey_c(iN,:) = wP .* r + (1-wP) .* Rprey_global(iN,:);

            % unchosen prey: matched samples only
            spikes_unprey = spk(idx_unprey,iN);

            [R, ~] = rate_map_with_occ(unpreyX(idx_unprey), unpreyY(idx_unprey), ...
                spikes_unprey, xedges, yedges, dt);
            r = reshape(R,1,[]);
...
lo = prctile(pos, pct);
bins_hi = find(pos >= hi);      % "right" or "bottom or "bottom right corner"
bins_lo = find(pos <= lo);      % "left"  or "top" or "top left corner"
...
for k = 1:K
    ...
    Xp_hi = Rprey(:, good_hi_P)';   % left bin prey on all the neurons
    Xp_lo = Rprey(:, good_lo_P)';   % right bin prey on all the neurons
    Xu_hi = Runprey(:, good_hi_U)';   % left bin prey on all the neurons
...
fprintf('Built items: Self=%d x %d | Prey=%d x %d\n', ...
    size(SelfItems,1), size(SelfItems,2), size(PreyItems,1), size(PreyItems,2));
...
fprintf('Class balance: Self high=%d low=%d | Prey high=%d low=%d\n', ...
    sum(ySelf==1), sum(ySelf==0), sum(yPrey==1), sum(yPrey==0));


%% Save data for CCGP decoding 
save(['chunkForCCGP_40s_6BESTbinsPerClass_' decoding '_rawTimeDistanceMatched1.mat'], ...
    'SelfItems', 'PreyItems', 'UnpreyItems', ...
    'ySelf', 'yPrey', 'yUnprey', ...
    'chunk_id_self', 'chunk_id_prey', 'chunk_id_unprey');
