...
for iN = 1 : n_neuron
    ...
    for iB = 1:nBoot
    grids = {self_pos_grid}; % concatenate all the maps
    ...
    % the code generates and error if there are no spikes for a neuron,
    ...
    % Gaussian filter for firing rate smoothing 
    filter = gaussmf(-15:15,[4 0]); filter = filter/sum(filter);
    ...
    stopflag = false;
		...
		while  num_var_in_model <= n_var 
            ...
            % find indeces for models with a number of variables equal to the number of num_var_in_model
            ...
            % index with the indeces off all the possible model
            % combinations that include the best single model 
			if num_var_in_model>1
				temp = find(all(modelType(:,find(modelType(selected_model,:)))==1,2)); % this variable contains the indeces of the best single models + the possible combination of that model
            ...
            % extract indeces of models to compare
        ...
        % so the model that best maximizes the increse LLH (LLH of null
